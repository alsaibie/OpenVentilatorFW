...
public:
	ActuatorManager() :
			OVThread("Actuator Manager ", 128, actuator_m_priority, 20), actuator_cmd_sub(
					gActuatorCommandsOVQHandle, &on_actuator_cmd_receive,
					Receive) {
	}
...
protected:
	virtual void run() {
		...
		update_motor_parameters(); // a parameter update call
		while (1) {
			/*
...
private:
	static void on_actuator_cmd_receive(
			const OVTopics::ActuatorCommands_msg_t &msg) {
